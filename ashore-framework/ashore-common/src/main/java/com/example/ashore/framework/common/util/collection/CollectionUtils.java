 ...
 * 目的：
 * - 提供强大的集合操作方法，简化业务代码
 * - 基于 Java Stream API 提供函数式编程风格的集合操作
 * - 封装 Hutool、Guava、Spring 等工具库的集合功能
 * - 提供类型安全的泛型方法，支持复杂的数据转换
 * - 避免空指针异常，所有方法都做了空值检查
 *
 * 功能分类：
 * - 集合判断：containsAny、isAnyEmpty、anyMatch
 * - 集合过滤：filterList、distinct（去重）
 * - 集合转换：convertList（List转换）、convertSet（Set转换）、convertMap（Map转换）
 * - 集合扁平化：convertListByFlatMap、convertSetByFlatMap（处理嵌套集合）
 * - 集合分组：convertMultiMap（一对多映射）
 * - 集合查找：findFirst、getFirst、getMaxValue、getMinValue
 * - 集合聚合：getSumValue（求和）
 * - 集合比较：diffList（对比新旧列表）
 *
 * see《CollectionUtils使用说明.md》for configuration details.
 */
