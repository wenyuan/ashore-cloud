...
/**
 * 分页查询工具类，提供了分页查询时排序字段构建的辅助方法
 * 目的：使用 Lambda 表达式获取字段名，避免硬编码字符串
 * {@link com.example.ashore.framework.common.pojo.PageParam} 工具类
 */
public class PageUtils {
    ...
    /**
     * 构建排序字段（指定排序）
     * 示例：SortingField sort = PageUtils.buildSortingField(UserDO::getId, SortingField.ORDER_ASC);
     *
    ...
    /**
     * 构建排序字段（默认倒序）
     * 示例：SortingField sort = PageUtils.buildSortingField(UserDO::getCreateTime);
     *
     * @param func 排序字段的 Lambda 表达式
     * @param <T>  排序字段所属的类型
     * @return 排序字段
