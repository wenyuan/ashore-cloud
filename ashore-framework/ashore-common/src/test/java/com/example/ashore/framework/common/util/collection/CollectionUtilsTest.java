...
public class CollectionUtilsTest {
    ...
    public void testDiffList() {
        ...
        assertEquals(result.get(0).get(0), new Dog(null, "果酱", "jsm"));
        // 断言 update
        assertEquals(result.get(1).size(), 1);
        assertEquals(result.get(1).get(0), new Dog(1, "花生2", "peanut"));
        // 断言 delete
        assertEquals(result.get(2).size(), 1);
        assertEquals(result.get(2).get(0), new Dog(2, "奶油", "cream"));
    }
