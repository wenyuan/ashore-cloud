package com.example.ashore.framework.security.core.filter;
...
import com.example.ashore.framework.security.config.SecurityProperties;
...
public class TokenAuthenticationFilter extends OncePerRequestFilter {
        ...
        if (loginUser == null) {
            ...
            if (StrUtil.isNotEmpty(token)) {
                ...
                try {
                    ...
                    ApiResponse<?> apiResponse = globalExceptionHandler.allExceptionHandler(request, ex);
                    ServletUtils.writeJSON(response, apiResponse);
                    return;
                }
            }
        }
    ...
    private LoginUser buildLoginUserByToken(String token, Integer userType) {
        try {
            // 校验访问令牌
            OAuth2AccessTokenCheckRespDTO accessToken = oauth2TokenApi.checkAccessToken(token).getDataOrThrow();
            if (accessToken == null) {
                return null;
            }
            // 用户类型不匹配，无权限
