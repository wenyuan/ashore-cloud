package com.example.ashore.framework.apilog.core.filter;
...
import com.example.ashore.framework.apilog.core.annotation.ApiAccessLog;
import com.example.ashore.framework.apilog.core.enums.OperateTypeEnum;
...
import com.example.ashore.framework.web.core.filter.ApiRequestFilter;
...
import static com.example.ashore.framework.apilog.core.interceptor.ApiAccessLogInterceptor.ATTRIBUTE_HANDLER_METHOD;
import static com.example.ashore.framework.common.util.json.JsonUtils.toJsonString;
...
public class ApiAccessLogFilter extends ApiRequestFilter {
    ...
    private boolean buildApiAccessLog(ApiAccessLogCreateReqDTO accessLog, HttpServletRequest request, LocalDateTime beginTime,
        ...
        // 设置访问结果
        ApiResponse<?> apiResponse = WebFrameworkUtils.getApiResponse(request);
        if (apiResponse != null) {
            accessLog.setErrorCode(apiResponse.getCode()).setResponseMsg(apiResponse.getMsg());
        } else if (ex != null) {
            accessLog.setErrorCode(GlobalErrorCodeConstants.INTERNAL_SERVER_ERROR.getCode())
                    .setResponseMsg(ExceptionUtil.getRootCauseMessage(ex));
        } else {
            accessLog.setErrorCode(GlobalErrorCodeConstants.SUCCESS.getCode()).setResponseMsg("");
        }
        // 设置请求字段
        ...
        if (BooleanUtil.isTrue(responseEnable)) { // 默认不记录，默认强制要求 true
            accessLog.setResponseBody(sanitizeJson(apiResponse, sanitizeKeys));
        }
        // 持续时间
    ...
    private static String sanitizeJson(ApiResponse<?> apiResponse, String[] sanitizeKeys) {
        if (apiResponse == null) {
            return null;
        }
        String jsonString = toJsonString(apiResponse);
        try {
            JsonNode rootNode = JsonUtils.parseTree(jsonString);
